...
map_expand()
{
    ...
    for (i=0;i<E.map.size;i++) {
        ...
        while (t) {
            ...
            const void *p=E.pool[tmp->id].u.n.mem;
...
map_erase(int id)
{
    const void *p=E.pool[id].u.n.mem;
    if (p) {
        int h=hash(p);
        struct hash_node **node= &E.map.table[h & (E.map.size -1)];
...
void
gc_link(const void *parent,const void *prev,const void *now)
{
    int parent_id;
    if (parent==0) {
        parent_id=0;
...
gc_exit()
{
    ...
    for (i=0;i<E.size;i++) {
        ...
        if (E.pool[i].mark >= 0) {
            printf("terminate %d(%p)\n",i,E.pool[i].u.n.mem);
            my_free((void *)E.pool[i].u.n.mem);
        }
    }
    my_free(E.pool);
    for (i=0;i<E.map.size;i++) {
...
void*
gc_clone(const void *from,size_t sz)
{
...
/* realloc a memory block , all the edages linked to the it will be retained , but you should relink it to others */

void* 
gc_realloc(const char *p,size_t sz)
{
    void *ret;
    assert(sz>0);
